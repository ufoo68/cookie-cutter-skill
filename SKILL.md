...
Use Blender MCP to build a printable cutter from a 2D outline, verify the geometry in Blender, and export an STL. Prefer the bundled Blender Python template for repeatable wall, blade, and export behavior.

## Workflow

1. Clarify the physical intent only when needed: overall size in mm, cutter height, wall thickness, blade thickness, whether an embossing stamp or handle lip is needed, and the target output path.
...
4. Inspect the created object with `mcp__blender__get_object_info`. Check dimensions, object count, and that the STL was exported to the requested path.
...
- Default dimensions: `height_mm=15`, `wall_width_mm=1.6`, `cut_width_mm=0.6`, `blade_height_mm=2.0`.
- Keep the cutter body one watertight mesh. A tapered lower blade should transition from `cut_width_mm` at the cutting edge to `wall_width_mm` above it.
- Add separate embossing or handle geometry only when requested. Name objects clearly, for example `cookie_cutter`, `emboss_lines`, and `handle_lip`.
...
When the user provides an image or vague idea, first derive a simple silhouette. State assumptions in the final response: size, height, wall thickness, and output path.
